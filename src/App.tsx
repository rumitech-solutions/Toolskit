...
function ToolControls({active,values,setV,setFiles}:{active:string;values:Values;setV:(k:string,v:string)=>void;setFiles:(f:File[])=>void}){const fileTools=tools.filter(t=>t.file).map(t=>t.id);if(fileTools.includes(active))return <div className="controls"><label className="file-drop">Choose local files<input type="file" multiple={['merge-pdf','jpg-to-pdf'].includes(active)} accept={active.includes('pdf')?'application/pdf':'image/*'} onChange={e=>setFiles(Array.from(e.target.files??[]))}/></label>{['split-pdf','extract-pdf-pages','delete-pdf-pages'].includes(active)&&<label>Pages<input value={values.pages} onChange={e=>setV('pages',e.target.value)}/></label>}{active==='reorder-pdf-pages'&&<label>Order<input value={values.order} onChange={e=>setV('order',e.target.value)}/></label>}{active==='rotate-pdf'&&<label>Angle<select value={values.angle} onChange={e=>setV('angle',e.target.value)}><option>90</option><option>180</option><option>270</option></select></label>}{active==='compress-pdf'&&<label>Quality<input type="range" min="30" max="95" value={values.quality} onChange={e=>setV('quality',e.target.value)}/></label>}{active==='watermark-pdf'&&<><label>Watermark text<input value={values.watermarkText} onChange={e=>setV('watermarkText',e.target.value)}/></label><label>Opacity %<input type="range" min="10" max="80" value={values.watermarkOpacity} onChange={e=>setV('watermarkOpacity',e.target.value)}/></label></>}{active==='image-cropper'?<div className="controls"><label>X<input type="number" min="0" value={values.cropX} onChange={e=>setV('cropX',e.target.value)}/></label><label>Y<input type="number" min="0" value={values.cropY} onChange={e=>setV('cropY',e.target.value)}/></label><label>Crop width<input type="number" min="1" value={values.cropWidth} onChange={e=>setV('cropWidth',e.target.value)} placeholder="required"/></label><label>Crop height<input type="number" min="1" value={values.cropHeight} onChange={e=>setV('cropHeight',e.target.value)} placeholder="required"/></label><label>Quality<input type="range" min="10" max="100" value={values.quality} onChange={e=>setV('quality',e.target.value)}/></label></div>:((['image-compressor','image-resizer','image-converter'].includes(active)||['jpg-to-png','png-to-jpg','webp-to-jpg','jpg-to-webp','png-to-webp'].includes(active))&&<div className="controls"><label>Quality<input type="range" min="10" max="100" value={values.quality} onChange={e=>setV('quality',e.target.value)}/></label>{active==='image-resizer'&&<><label>Width<input type="number" min="1" value={values.width} onChange={e=>setV('width',e.target.value)} placeholder="auto"/></label><label>Height<input type="number" min="1" value={values.height} onChange={e=>setV('height',e.target.value)} placeholder="auto"/></label></>}</div>)}
 ...
 if(active==='slug-generator'||active==='word-frequency-counter'||active==='html-tag-remover'||active==='csv-to-json'||active==='color-converter'||active==='image-metadata')return null
